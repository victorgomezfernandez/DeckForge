    ...
    <div class="container details-container">
            ...
            @if (auth()->check() && auth()->user()->id === $deck->user_id)
                <div class="col-lg-4 col-sm-6 live-search-column">
                    <div class="live-search-input-container">
                        <label for="card-live-search" class="live-search-label"><b>{{ __('decks.add_cards') }}</b></label>
        ...
        <div class="row mt-2">
            <div class="deck-details-header">
                <div class="deck-details-colors-name">
                    <div id="deck-colors">
                        @include('components.deck-details-colors', ['deck' => $deck])
                    </div>
                    @if (auth()->check() && auth()->user()->id === $deck->user_id)
                        <textarea id="deckNameInput" class="deck-details-name deck-details-input" rows="1">{{ $deck->name }}</textarea>
                    @else
        ...
        <div class="deck-details-description mt-2">
            @if (auth()->check() && auth()->user()->id === $deck->user_id)
                <textarea class="deck-details-description deck-details-input-description" placeholder="Deck description"
                    id="deckDescriptionInput" rows="3">{{ $deck->description }}</textarea>
            @else
